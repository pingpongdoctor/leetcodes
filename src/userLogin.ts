...
const returnValidIds: ReturnValidIds = function (inforArr, maxSpan) {
  ...
  const isMaxSpanValid =
    Number.isInteger(maxSpan) && 0 < maxSpan && maxSpan <= 200;

  if (!isInforArrValid || !isMaxSpanValid) {
    return "Invalid input";
  }

  const validIdArr: number[] = [];
